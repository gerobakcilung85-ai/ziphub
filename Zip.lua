--[[
    ZIP Interface Suite
    by ZIP (based on Rayfield by Sirius)
]]

if debugX then
    warn('Initialising ZIP')
end
...
local customAssetId = 128536767801427  -- GANTI DENGAN ID ASSET ANDA
local secureMode = false
if getgenv then
    local ok, result = pcall(function() return getgenv().DISABLE_ZIP_REQUESTS end)
    if ok and result then requestsDisabled = true end
    local ok2, result2 = pcall(function() return getgenv().ZIP_ASSET_ID end)
    if ok2 and type(result2) == "number" then customAssetId = result2 end
    local ok3, result3 = pcall(function() return getgenv().ZIP_SECURE end)
...
local function secureNotify(wType, title, content)
    ...
    task.spawn(function()
        while not ZIPLibrary or not ZIPLibrary.Notify do task.wait(0.5) end
        ZIPLibrary:Notify({
            Title = title,
            Content = content,
            Duration = 8,
...
local InterfaceBuild = 'ZIPBUILD'
local Release = "ZIP Edition v1.0"
local ZIPFolder = "ZIP"
local ConfigurationFolder = ZIPFolder.."/Configurations"
local ConfigurationExtension = ".zipcfg"
local settingsTable = {
    General = {
        zipOpen = {Type = 'bind', Value = 'K', Name = 'ZIP Keybind'},
    },
    System = {
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.zipOpen"] = "J"
...
local function callSafely(func, ...)
    if func then
        ...
        if not success then
            warn("ZIP | Function failed with error: ", result)
            return false
        else
            return result
        end
...
local function loadSettings()
    ...
    local success, result =    pcall(function()
        if callSafely(isfolder, ZIPFolder) then
            if callSafely(isfile, ZIPFolder..'/settings'..ConfigurationExtension) then
                file = callSafely(readfile, ZIPFolder..'/settings'..ConfigurationExtension)
            end
        end
        ...
        if useStudio then
            file = [[
    {"General":{"zipOpen":{"Value":"K","Type":"bind","Name":"ZIP Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"ZIP Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
]]
        end
            ...
            for categoryName, categoryTable in file do
                for settingName, setting in categoryTable do
                    ...
                    if not (settingType == typeof(setting.Value)) then
                        warn("ZIP | Error parsing settings file. '"..settingName.."' must be a "..settingType)
                        continue
                    end
    ...
    if not success then
        if writefile then
            warn('ZIP had an issue accessing configuration saving capability.')
        end
    end
end
...
local ZIPAssetId = customAssetId or 128536767801427
local ZIPUI = useStudio and script.Parent:FindFirstChild('ZIP') or game:GetObjects("rbxassetid://"..ZIPAssetId)[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local zipDestroyed = false -- True when ZIPLibrary:Destroy() is called
repeat
    if ZIPUI:FindFirstChild('Build') and ZIPUI.Build.Value == InterfaceBuild then
        correctBuild = true
        break
    end
    correctBuild = false
    if not warned then
        warn('ZIP | Build Mismatch')
        print('ZIP may encounter issues as you are running an incompatible interface version ('.. ((ZIPUI:FindFirstChild('Build') and ZIPUI.Build.Value) or 'No Build') ..').\n\nThis version of ZIP is intended for interface build '..InterfaceBuild..'.')
        warned = true
    end
    local toDestroy
    toDestroy, ZIPUI = ZIPUI, useStudio and script.Parent:FindFirstChild('ZIP') or game:GetObjects("rbxassetid://"..ZIPAssetId)[1]
...
elseif syn and syn.protect_gui then
    syn.protect_gui(ZIPUI)
    ZIPUI.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
    ZIPUI.Parent = CoreGui:FindFirstChild("RobloxGui")
elseif not useStudio then
    ZIPUI.Parent = CoreGui
end
if gethui then
    for _, Interface in ipairs(gethui():GetChildren()) do
        if Interface.Name == ZIPUI.Name and Interface ~= ZIPUI then
            Interface.Enabled = false
            Interface.Name = "ZIP-Old"
        end
    end
elseif not useStudio then
...
if secureMode and not customAssetId then
    secureNotify("default_asset", "Secure Mode", "You are using the default ZIP asset ID. Set ZIP_ASSET_ID to a custom upload to avoid detection.")
end
do
    local AssetPath = ZIPFolder.."/Assets"
        ...
        local ok, err = pcall(function()
            ...
            for id, _ in assetFiles do
                ...
                if success then
                    ...
                    warn("ZIP | Failed to load custom asset: "..tostring(id).." - "..tostring(asset))
                end
            end
        end)
        if not ok then
            warn("ZIP | Failed to load custom assets: "..tostring(err))
            secureNotify("asset_load_fail", "ZIP", "Failed to load custom assets. UI images may not display correctly.")
        end
    else
        secureNotify("no_getcustomasset", "ZIP", "Your executor does not support getcustomasset. Some UI images may not render correctly.")
    end
    ZIPUI.Main.Shadow.Image.Image = customAssets[tostring(5587865193)]
    ZIPUI.Main.Topbar.Hide.Image = customAssets[tostring(10137832201)]
    ZIPUI.Main.Topbar.ChangeSize.Image = customAssets[tostring(10137941941)]
    ZIPUI.Main.Topbar.Settings.Image = customAssets[tostring(80503127983237)]
    ZIPUI.Main.Topbar.Icon.Image = customAssets[tostring(78137979054938)]
    ZIPUI.Main.Topbar.Search.Image = customAssets["IconMagnifyingGlass2"]
    ZIPUI.Main.Topbar.Search.ImageRectOffset = Vector2.new(0, 0)
    ZIPUI.Main.Topbar.Search.ImageRectSize = Vector2.new(0, 0)
    ZIPUI.Main.Elements.Template.Toggle.Switch.Shadow.Image = customAssets[tostring(3602733521)]
    ZIPUI.Main.Elements.Template.Slider.Main.Shadow.Image = customAssets[tostring(3602733521)]
    ZIPUI.Main.Elements.Template.Dropdown.Toggle.Image = customAssets["IconChevronTopMedium"]
    ZIPUI.Main.Elements.Template.Dropdown.Toggle.ImageRectOffset = Vector2.new(0, 0)
    ZIPUI.Main.Elements.Template.Dropdown.Toggle.ImageRectSize = Vector2.new(0, 0)
    ZIPUI.Main.Elements.Template.Label.Icon.Image = customAssets[tostring(11745872910)]
    ZIPUI.Main.Elements.Template.ColorPicker.CPBackground.MainCP.Image = customAssets[tostring(11413591840)]
    ZIPUI.Main.Elements.Template.ColorPicker.CPBackground.MainCP.MainPoint.Image = customAssets[tostring(3259050989)]
    ZIPUI.Main.Elements.Template.ColorPicker.ColorSlider.SliderPoint.Image = customAssets[tostring(3259050989)]
    ZIPUI.Main.TabList.Template.Image.Image = customAssets[tostring(4483362458)]
    ZIPUI.Main.Search.Search.Image = customAssets[tostring(18458939117)]
    ZIPUI.Main.Search.Shadow.Image = customAssets[tostring(5587865193)]
    ZIPUI.Notifications.Template.Icon.Image = customAssets[tostring(77891951053543)]
    ZIPUI.Notifications.Template.Shadow.Image = customAssets[tostring(3523728077)]
    ZIPUI.Loading.Banner.Image = customAssets[tostring(111263549366178)]
end -- custom asset block

local minSize = Vector2.new(1024, 768)
local useMobileSizing
if ZIPUI.AbsoluteSize.X < minSize.X and ZIPUI.AbsoluteSize.Y < minSize.Y then
...
-- Object Variables
local Main = ZIPUI.Main
local MPrompt = ZIPUI:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = ZIPUI:FindFirstChild('Drag')
local dragInteract = dragBar and dragBar.Interact or nil
local dragBarCosmetic = dragBar and dragBar.Drag or nil
local dragOffset = 255
local dragOffsetMobile = 150
ZIPUI.DisplayOrder = 100
...
local Notifications = ZIPUI.Notifications
local keybindConnections = {} -- For storing keybind connections to disconnect when ZIP is destroyed
local SelectedTheme = ZIPLibrary.Theme.Default

local function ChangeTheme(Theme)
    if typeof(Theme) == 'string' then
        SelectedTheme = ZIPLibrary.Theme[Theme]
    elseif typeof(Theme) == 'table' then
        SelectedTheme = Theme
    end
    ZIPUI.Main.BackgroundColor3 = SelectedTheme.Background
    ZIPUI.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
    ZIPUI.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
    ZIPUI.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow
    ZIPUI.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
    ZIPUI.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
    ZIPUI.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
    if Topbar:FindFirstChild('Settings') then
        ZIPUI.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
        ZIPUI.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
    end
    ...
    for _, text in ipairs(ZIPUI:GetDescendants()) do
...
local function getAssetUri(id: any): string
    ...
    elseif type(id) == "string" and not Icons then
        warn("ZIP | Cannot use Lucide icons as icons library is not loaded")
    else
        warn("ZIP | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
    end
    return assetUri
end
...
local function LoadConfiguration(Configuration)
    ...
    local changed
    if not success then warn('ZIP had an issue decoding the configuration file, please try delete the file and reopen ZIP.') return end
    -- Iterate through current UI elements' flags
    for FlagName, Flag in pairs(ZIPLibrary.Flags) do
        ...
        if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
            ...
            warn("ZIP | Unable to find '"..FlagName.. "' in the save file.")
            print("The error above may not be an issue if new elements have been added or not been set values.")
            --ZIPLibrary:Notify({Title = "ZIP Flags", Content = "ZIP was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
        end
    end
    return changed
end
...
local function SaveConfiguration()
    ...
    local Data = {}
    for i, v in pairs(ZIPLibrary.Flags) do
...
function ZIPLibrary:Notify(data) -- action e.g open messages
    task.spawn(function()
        ...
        if data.Actions then
            warn('ZIP | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
    ...
    if notify then
        if useMobilePrompt then
            ZIPLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
        else
            ZIPLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "zipOpen")) .. ".", Duration = 7, Image = 4400697855})
        end
    end
...
local function saveSettings() -- Save settings to config file
    ...
    if success then
        ...
        callSafely(writefile, ZIPFolder..'/settings'..ConfigurationExtension, encoded)
    end
end
...
local function createSettings(window)
    ...
    local newTab = window:CreateTab('ZIP Settings', 0, true)
    if TabList['ZIP Settings'] then
        TabList['ZIP Settings'].LayoutOrder = 1000
    end
    if Elements['ZIP Settings'] then
        Elements['ZIP Settings'].LayoutOrder = 1000
    end
...
function ZIPLibrary:CreateWindow(Settings)
    if ZIPUI:FindFirstChild('Loading') then
        if getgenv and not getgenv().zipCached then
            ZIPUI.Enabled = true
            ZIPUI.Loading.Visible = true
            task.wait(1.4)
            ZIPUI.Loading.Visible = false
        end
    end
    if getgenv then getgenv().zipCached = true end
    if not correctBuild and not Settings.DisableBuildWarnings then
        task.delay(3,
            function()
                ZIPLibrary:Notify({Title = 'Build Mismatch', Content = 'ZIP may encounter issues as you are running an incompatible interface version ('.. ((ZIPUI:FindFirstChild('Build') and ZIPUI.Build.Value) or 'No Build') ..').\n\nThis version of ZIP is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})
            end)
    end
    ...
    LoadingFrame.Title.Text = Settings.LoadingTitle or "ZIP"
    LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"
    if Settings.LoadingTitle ~= "ZIP Interface Suite" then
        LoadingFrame.Version.Text = "ZIP UI"
    end
    ...
    if not Settings.DisableZIPPrompts then
        task.spawn(function()
            while not zipDestroyed do
                task.wait(math.random(180, 600))
                if zipDestroyed then break end
                ZIPLibrary:Notify({
                    Title = "ZIP Interface",
    ...
    if Settings.Discord and Settings.Discord.Enabled and not useStudio and not secureMode then
        ensureFolder(ZIPFolder.."/Discord Invites")
        if not callSafely(isfile, ZIPFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
            if requestFunc then
                pcall(function()
                    requestFunc({
            ...
            if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
                callSafely(writefile, ZIPFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"ZIP RememberJoins is true for this invite, this invite will not ask you to join again")
            end
        end
    end
    if (Settings.KeySystem) then
        ...
        ensureFolder(ZIPFolder.."/Key System")
        ...
        if Settings.KeySettings.GrabKeyFromSite then
            for i, Key in ipairs(Settings.KeySettings.Key) do
                ...
                if not Success then
                    print("ZIP | "..Key.." Error " ..tostring(Response))
                    warn('Check docs.sirius.menu for help with ZIP specific development.')
                end
            end
        end
        ...
        if callSafely(isfile, ZIPFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
            for _, MKey in ipairs(Settings.KeySettings.Key) do
                local savedKeys = callSafely(readfile, ZIPFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
        ...
        if not Passthrough and secureMode then
            warn("ZIP | Secure Mode: Key system requires a valid saved key. The key UI cannot be shown as it requires loading detectable assets.")
            ZIPUI.Enabled = false
            return ZIPLibrary
        end
            ...
            KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
                ...
                if KeyFound then
                    ...
                    if Settings.KeySettings.SaveKey then
                        callSafely(writefile, ZIPFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
                        ZIPLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
                    end
                else
    ...
    if Settings.KeySystem then
        ...
        if zipDestroyed then return end
    end
    ...
    function Window:CreateTab(Name, Image, Ext)
        ...
        function Tab:CreateButton(ButtonSettings)
            ...
            Button.Interact.MouseButton1Click:Connect(function()
                ...
                -- Prevents animation from trying to play if the button's callback called ZIPLibrary:Destroy()
                if zipDestroyed then
                    return
                end
                if not Success then
                    ...
                    print("ZIP | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
                    warn('Check docs.sirius.menu for help with ZIP specific development.')
                    task.wait(0.5)
                    Button.Title.Text = ButtonSettings.Name
        ...
        function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
            ...
            ColorPicker.Interact.MouseButton1Down:Connect(function()
                ...
                if not opened then
                    ...
                    TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= ZIPLibrary.Theme.Default and 0.25 or 0.1}):Play()
            ...
            if Settings.ConfigurationSaving then
                if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
                    ZIPLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
                end
            end
            ...
            ZIPUI.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
                for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
                    if rgbinput:IsA("Frame") then
                        rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
                        rgbinput.UIStroke.Color = SelectedTheme.InputStroke
        ...
        function Tab:CreateInput(InputSettings)
            ...
            Input.InputFrame.InputBox.FocusLost:Connect(function()
                ...
                if not Success then
                    ...
                    print("ZIP | "..InputSettings.Name.." Callback Error " ..tostring(Response))
                    warn('Check docs.sirius.menu for help with ZIP specific development.')
                    task.wait(0.5)
                    Input.Title.Text = InputSettings.Name
            ...
            if Settings.ConfigurationSaving then
                if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
                    ZIPLibrary.Flags[InputSettings.Flag] = InputSettings
                end
            end
            ZIPUI.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
                Input.InputFrame.BackgroundColor3 = SelectedTheme.InputBackground
        ...
        function Tab:CreateDropdown(DropdownSettings)
                ...
                for _, Option in ipairs(DropdownSettings.Options) do
                    ...
                    DropdownOption.Interact.MouseButton1Click:Connect(function()
                        ...
                        if not Success then
                            ...
                            print("ZIP | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
                            warn('Check docs.sirius.menu for help with ZIP specific development.')
                            task.wait(0.5)
                            Dropdown.Title.Text = DropdownSettings.Name
                            TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
            ...
            if Settings.ConfigurationSaving then
                if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
                    ZIPLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
                end
            end
            ZIPUI.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
                Dropdown.Toggle.ImageColor3 = SelectedTheme.TextColor
        ...
        function Tab:CreateKeybind(KeybindSettings)
                ...
                elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
                    ...
                    if not KeybindSettings.HoldToInteract then
                        ...
                        if not Success then
                            ...
                            print("ZIP | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
                            warn('Check docs.sirius.menu for help with ZIP specific development.')
                            task.wait(0.5)
                            Keybind.Title.Text = KeybindSettings.Name
                            TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
            ...
            if Settings.ConfigurationSaving then
                if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
                    ZIPLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
                end
            end
            ZIPUI.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
                Keybind.KeybindFrame.BackgroundColor3 = SelectedTheme.InputBackground
        ...
        function Tab:CreateToggle(ToggleSettings)
            ...
            if SelectedTheme ~= ZIPLibrary.Theme.Default then
            ...
            Toggle.Interact.MouseButton1Click:Connect(function()
                ...
                if not Success then
                    ...
                    print("ZIP | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
                    warn('Check docs.sirius.menu for help with ZIP specific development.')
                    task.wait(0.5)
                    Toggle.Title.Text = ToggleSettings.Name
                    TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
            ...
            if not ToggleSettings.Ext then
                if Settings.ConfigurationSaving then
                    if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
                        ZIPLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
                    end
                end
            end
            ZIPUI.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
        ...
        function Tab:CreateSlider(SliderSettings)
                    ...
                    if SLDragging then
                        ...
                        if SliderSettings.CurrentValue ~= NewValue then
                            ...
                            if not Success then
                                ...
                                print("ZIP | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
                                warn('Check docs.sirius.menu for help with ZIP specific development.')
                                task.wait(0.5)
                                Slider.Title.Text = SliderSettings.Name
                                TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
            ...
            if Settings.ConfigurationSaving then
                if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
                    ZIPLibrary.Flags[SliderSettings.Flag] = SliderSettings
                end
            end
            ZIPUI.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
                if SelectedTheme ~= ZIPLibrary.Theme.Default then
    ...
    function Window.ModifyTheme(NewTheme)
        ...
        if not success then
            ZIPLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
        else
            ZIPLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
        end
    end
    local success, result = pcall(function()
        createSettings(Window)
    end)
    if not success then warn('ZIP had an issue creating settings.') end
...
function ZIPLibrary:SetVisibility(visibility: boolean)
    setVisibility(visibility, false)
end

function ZIPLibrary:IsVisible(): boolean
    return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script

function ZIPLibrary:Destroy()
    zipDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
    Topbar.Settings.MouseButton1Click:Connect(function()
        task.spawn(function()
            ...
            Elements.UIPageLayout:JumpTo(Elements['ZIP Settings'])
        end)
    end)
end
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
    if (input.KeyCode == Enum.KeyCode[getSetting("General", "zipOpen")]) and not processed then
        if Debounce then return end
        if Hidden then
            Hidden = false
            Unhide()
...
function ZIPLibrary:LoadConfiguration()
    local config
    if debugX then
    ...
    if CEnabled then
        ...
        local success, result = pcall(function()
            ...
            if isfile then
                ...
                notified = true
                ZIPLibrary:Notify({Title = "ZIP Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
            end
        end)
        if success and loaded and not notified then
            ZIPLibrary:Notify({Title = "ZIP Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
        elseif not success and not notified then
            warn('ZIP Configurations Error | '..tostring(result))
            ZIPLibrary:Notify({Title = "ZIP Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
        end
    end
    globalLoaded = true
end
...
if useStudio then
    -- run w/ studio
    -- (contoh penggunaan di studio, bisa diisi sesuai kebutuhan)
end
